...
import ErrorComponent from "@/app/(Pages Group)/restaurant/[slug]/components/ErrorComponent";
...
const Form = ({ slug, date, partySize }: { slug: string; date: string; partySize: string }) => {
    ...
    setInput((prev) => {
      if (session) {
        return {
          ...
          bookerPhone: session.user.phone,
  ...
  useEffect(() => {
    if (input.bookerName && input.bookerPhone && input.bookerEmail) {
      setIsDisabled(false);
      return;
    }
    setIsDisabled(true);
